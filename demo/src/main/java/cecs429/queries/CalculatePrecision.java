...
public class CalculatePrecision {
    private static Indexer indexSearch = new Indexer();
    private double queryTime = 0.0; //timing purposes
    //This calculates average precision
    public double averagePrecision(DocumentCorpus corpus, Index index, String query, int[] relDocs) {
        ...
        PriorityQueue<Accumulator> pq = indexSearch.rankedSearch(corpus, index, query);
        ...
        indexSearch.setQueryTime(queryTime + queryRuntime);
    ...
    public static void meanAveragePrecision(String path, DocumentCorpus corpus, Index index) {
        ...
        for (int i = 0; i < queries.size(); i++) {//run the AP formula
            sumAvgPrecision += indexSearch.averagePrecision(corpus, index, queries.get(i), false, false, relDocs.get(i));
        }
